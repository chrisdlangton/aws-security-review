...
def cis_avoid_the_use_of_the_root_account(account, rule_config):
  ...
  if response == 'COMPLETE':
    ...
    for s in users:
      ...
      password_last_used = helpers.from_iso8601(report['password_last_used'])
      delta = now - password_last_used
      if delta.days <= rule_config['settings']['password_used']:
        break

      if report['access_key_1_active'] == 'true':
        access_key_1_last_used_date = helpers.from_iso8601(report['access_key_1_last_used_date'])
      ...
      if report['access_key_2_active'] == 'true':
        access_key_2_last_used_date = helpers.from_iso8601(report['access_key_2_last_used_date'])
